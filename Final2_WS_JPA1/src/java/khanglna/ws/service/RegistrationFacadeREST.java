...
import khanglna.ws.Registration;
 ...
 */
@Stateless
@Path("khanglna.ws.registration")
public class RegistrationFacadeREST extends AbstractFacade<Registration> {

    @PersistenceContext(unitName = "Final2_WS_JPA1PU")
    ...
    @Produces({MediaType.TEXT_PLAIN})
    public String checkLogin(@PathParam("user") String username,
            @PathParam("pass") String pass) {
        return super.checkLogin(username, pass);
    }

    @GET
    @Path("/findByLikeName/{fullname}")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<Registration> findByLikeName(@PathParam("fullname") String fname) {
        return super.findByLikeName(fname);
